...
Команда автоматически создаёт/подключает образ диска `build/myos_disk.img` (8 МиБ, ATA/IDE). При запуске MyOs пытается прочитать снимок ФС из этого диска; при отсутствии снимка создаётся дефолтная структура и сразу сохраняется.
